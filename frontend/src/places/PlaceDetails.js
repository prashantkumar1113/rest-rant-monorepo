...
function PlaceDetails() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch(
                `http://localhost:${process.env.REACT_APP_SERVER_PORT}/places/${placeId}`
            );
    ...
    async function deleteComment(deletedComment) {
        await fetch(
            `http://localhost:${process.env.REACT_APP_SERVER_PORT}/places/${place.placeId}/comments/${deletedComment.commentId}`,
            {
                method: "DELETE",
            }
        );
    ...
    async function createComment(commentAttributes) {
        const response = await fetch(
            `http://localhost:${process.env.REACT_APP_SERVER_PORT}/places/${place.placeId}/comments`,
            {
                method: "POST",
                headers: {
        ...
        <main>
            <div className="row">
                ...
                <div className="col-sm-6">
                    ...
                    <h2>Description</h2>
                    <h3>
                        {place.name} has been serving {place.city},{" "}
                        {place.state} since {place.founded}.
                    </h3>
